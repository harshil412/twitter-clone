...
export const deletePost = async (req, res) => {
    try {
        ...
        if(post.user.toString()!== req.user._id.toString()) return res.status(403).json({ message: "Unauthorized to delete this post"});
...
export const likeUnlikePost = async (req, res) => {
    try {
        ...
        if(userLikedPost){
            ...
            res.status(200).json({message : "Post linked successfully"});
        }
    } catch (error) {
        res.status(500).json({ error : error.message });
